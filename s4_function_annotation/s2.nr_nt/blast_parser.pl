...
#		print $cycle[0];
		...
		if ($cycle[0]=~/ \S+\s+.*Length=[\d\,]+\s+\*\*\*\*\* No hits found \*\*\*\*\*.* (\S+)\s+(.*)Length=([\d\,]+)/s) {
		 $query=$1;
		 $query_annotation=$2;
		 $query_len=$3;
		 $query_len =~ s/,//g;
		...
		# elsif ($cycle[0]=~/Query= (\S+)\s+(.*)Length=([\d\,]+)\s+$/s) {
		#       $query=$1;
		#	$query_annotation=$2;
		#	$query_len=$3;
		#	$query_len =~ s/,//g;
		#	$query_annotation =~ s/\s+/ /g;
		#	$query_annotation="--" if(!$query_annotation || $query_annotation eq " ");print $_;
		#}
		#	print $cycle[0];
		shift @cycle;
		for (my $i=0; $i<@cycle; $i++) {
			...
			for (my $j=0; $j<@cycle_inner; $j++) {
				...
				#	$cycle_inner[$j]=~s/\n\s*Query=\s*.+?$//s;	#最后一组情况中需删除文件末尾的某些多余信息
				$cycle_inner[$j]=~s/\s+$//s;
